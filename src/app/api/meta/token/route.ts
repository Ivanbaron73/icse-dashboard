import { NextRequest, NextResponse } from 'next/server'

const BASE = 'https://graph.facebook.com'

// GET /api/meta/token → verifica validez del token actual
export async function GET() {
  const token = process.env.META_ACCESS_TOKEN
  if (!token) {
    return NextResponse.json({ error: 'META_ACCESS_TOKEN no configurado' }, { status: 400 })
  }

  try {
    // Usar app access token si está disponible (más fiable con tokens expirados)
    const appId = process.env.META_APP_ID
    const appSecret = process.env.META_APP_SECRET
    const appToken = appId && appSecret ? `${appId}|${appSecret}` : token

    const res = await fetch(
      `${BASE}/debug_token?input_token=${token}&access_token=${appToken}`,
      { cache: 'no-store' }
    )
    const data = await res.json()

    if (data.error) {
      return NextResponse.json({ valid: false, error: data.error.message })
    }

    const info = data.data ?? {}
    const expiresAt = info.expires_at
      ? new Date(info.expires_at * 1000).toISOString()
      : null
    const daysLeft = expiresAt
      ? Math.floor((new Date(expiresAt).getTime() - Date.now()) / 86_400_000)
      : null

    return NextResponse.json({
      valid: info.is_valid ?? false,
      expiresAt,
      daysLeft,
      scopes: info.scopes ?? [],
      type: info.type ?? 'unknown',
    })
  } catch (error) {
    return NextResponse.json(
      { valid: false, error: error instanceof Error ? error.message : 'Error desconocido' },
      { status: 500 }
    )
  }
}

async function saveToVercel(newToken: string): Promise<boolean> {
  const vercelToken = process.env.VERCEL_API_TOKEN
  const projectId = process.env.VERCEL_PROJECT_ID ?? 'prj_oXhOOKM1iu1wWdPHgorQeBcEVqLZ'
  if (!vercelToken) return false
  try {
    const listRes = await fetch(
      `https://api.vercel.com/v9/projects/${projectId}/env`,
      { headers: { Authorization: `Bearer ${vercelToken}` } }
    )
    const listData = await listRes.json()
    const envVar = (listData.envs ?? []).find((e: { key: string }) => e.key === 'META_ACCESS_TOKEN')
    if (!envVar?.id) return false
    await fetch(`https://api.vercel.com/v9/projects/${projectId}/env/${envVar.id}`, {
      method: 'PATCH',
      headers: { Authorization: `Bearer ${vercelToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ value: newToken }),
    })
    return true
  } catch {
    return false
  }
}

// POST /api/meta/token
// Body { token, direct: true }  → guarda el token tal cual en Vercel (tokens permanentes)
// Body { shortLivedToken }      → intercambia por Long-Lived Token (60 días) y guarda
export async function POST(req: NextRequest) {
  const body = await req.json().catch(() => ({}))

  // ── Modo directo: guardar sin conversión ──
  if (body.direct && body.token) {
    const vercelUpdated = await saveToVercel(body.token)
    return NextResponse.json({
      success: true,
      token: body.token,
      daysLeft: null,
      permanent: true,
      vercelUpdated,
    })
  }

  // ── Modo exchange: token corto → Long-Lived (60 días) ──
  const appId = body.appId ?? process.env.META_APP_ID
  const appSecret = body.appSecret ?? process.env.META_APP_SECRET
  const shortToken = body.shortLivedToken ?? process.env.META_ACCESS_TOKEN

  if (!appId || !appSecret) {
    return NextResponse.json(
      { error: 'Se requieren META_APP_ID y META_APP_SECRET en las variables de entorno.' },
      { status: 400 }
    )
  }
  if (!shortToken) {
    return NextResponse.json({ error: 'No hay token para intercambiar.' }, { status: 400 })
  }

  try {
    const url =
      `${BASE}/oauth/access_token` +
      `?grant_type=fb_exchange_token` +
      `&client_id=${appId}` +
      `&client_secret=${appSecret}` +
      `&fb_exchange_token=${shortToken}`

    const res = await fetch(url, { cache: 'no-store' })
    const data = await res.json()

    if (data.error) {
      return NextResponse.json({ success: false, error: data.error.message }, { status: 400 })
    }

    const newToken: string = data.access_token
    const expiresInSeconds: number = data.expires_in ?? 5_184_000
    const daysLeft = Math.floor(expiresInSeconds / 86_400)
    const vercelUpdated = await saveToVercel(newToken)

    return NextResponse.json({ success: true, token: newToken, daysLeft, vercelUpdated })
  } catch (error) {
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Error desconocido' },
      { status: 500 }
    )
  }
}
